...
2026-09-14。全站 build **18,726 個檔案**，sitemap 收錄 **4,211** 個網址。
...
| `/search` | 1 | — | 即時搜尋，支援 `?q=` |
| `/about` | 1 | ✓ | 資料流程，數字全部實算 |
| `/event/{slug}` | 5,598 | 1,788 | schema.org Event、多來源並排、原始出處 |
| `/venue/{slug}` | 10,423 | 237 | schema.org Place，registry 與 derived 分開標示 |
| `/venues` | 1 | ✓ | 依活動數排序 |
...
| — | 測試 248 條（純函式 212 ＋ pipeline 各階段 36），`npm test` |

---

## 資料現況

79 支來源、71,926 筆 observation、5,598 個活動頁。

活動只有 32% 收錄，是因為 5,598 個裡多數已經結束——那正是品質分數要做的事。
**未結束的 2,646 個裡有 1,780 個夠格收錄（67%）。**

場館只有 2% 收錄，是因為 10,423 個裡絕大多數是沒有活動的名錄 POI（公共藝術、
